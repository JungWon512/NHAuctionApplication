package com.nh.controller.model;

/**
 * 수수료 데이터
 * 
 * @author jhlee
 *
 */
public class FeeImpsData {

	private String naBzplc;			// 경제통합사업장코드
	private String aucObjDsc;		// 경매대상구분코드
	private String aucDt;			// 경매일자
	private String oslpNo;			// 원표번호
	private String ledSqNo;			// 원장일련번호
	private String feeRgSqno;		//경제통합수수료코드
	private String aplDt;
	private String naFee_c;
	private String feeAplObj_c;
	private String ansDsc;
	private String sBidYn;
	private int sraTrFee;
	private String tmsYn;

	public String getNaBzplc() {
		return naBzplc;
	}

	public void setNaBzplc(String naBzplc) {
		this.naBzplc = naBzplc;
	}

	public String getAucObjDsc() {
		return aucObjDsc;
	}

	public void setAucObjDsc(String aucObjDsc) {
		this.aucObjDsc = aucObjDsc;
	}

	public String getAucDt() {
		return aucDt;
	}

	public void setAucDt(String aucDt) {
		this.aucDt = aucDt;
	}

	public String getOslpNo() {
		return oslpNo;
	}

	public void setOslpNo(String oslpNo) {
		this.oslpNo = oslpNo;
	}

	public String getLedSqNo() {
		return ledSqNo;
	}

	public void setLedSqNo(String ledSqNo) {
		this.ledSqNo = ledSqNo;
	}

	public String getFeeRgSqno() {
		return feeRgSqno;
	}

	public void setFeeRgSqno(String feeRgSqno) {
		this.feeRgSqno = feeRgSqno;
	}

	public String getAplDt() {
		return aplDt;
	}

	public void setAplDt(String aplDt) {
		this.aplDt = aplDt;
	}

	public String getNaFee_c() {
		return naFee_c;
	}

	public void setNaFee_c(String naFee_c) {
		this.naFee_c = naFee_c;
	}

	public String getFeeAplObj_c() {
		return feeAplObj_c;
	}

	public void setFeeAplObj_c(String feeAplObj_c) {
		this.feeAplObj_c = feeAplObj_c;
	}

	public String getAnsDsc() {
		return ansDsc;
	}

	public void setAnsDsc(String ansDsc) {
		this.ansDsc = ansDsc;
	}

	public String getsBidYn() {
		return sBidYn;
	}

	public void setsBidYn(String sBidYn) {
		this.sBidYn = sBidYn;
	}

	public int getSraTrFee() {
		return sraTrFee;
	}

	public void setSraTrFee(int sraTrFee) {
		this.sraTrFee = sraTrFee;
	}

	public String getTmsYn() {
		return tmsYn;
	}

	public void setTmsYn(String tmsYn) {
		this.tmsYn = tmsYn;
	}

}