...
public final class AuctionServerConnectorHandler extends SimpleChannelInboundHandler<ConnectionInfo> {
	...
	protected void channelRead0(ChannelHandlerContext ctx, ConnectionInfo connectionInfo) throws Exception {
		...
		if(GlobalDefineCode.FLAG_TEST_MODE) {
			...
			userMemNum = connectionInfo.getAuctionHouseCode() + "_" + JwtCertTokenUtils.getInstance().getUserMemNum(connectionInfo.getAuthToken());
		}
		...
		if (userMemNum != null) {
			mLogger.info("mConnectionInfoMap.containsKey(ctx.channel().id()) : " + mConnectionInfoMap.containsKey(ctx.channel().id()));
			mLogger.info("mConnectionInfoMap.containsValue(connectionInfo) : " + mConnectionInfoMap.containsValue(connectionInfo));
			mLogger.info("mConnectionChannelInfoMap.containsKey(userMemNum) : " + mConnectionChannelInfoMap.containsKey(userMemNum));
					...
					if (mControllerChannelsMap != null
						...
						if (!mConnectionInfoMap.containsKey(ctx.channel().id())) {	
							...
							if (GlobalDefineCode.FLAG_TEST_MODE) {
								...
								mConnectionChannelInfoMap.put(
										connectionInfo.getAuctionHouseCode() + "_" + JwtCertTokenUtils.getInstance().getUserMemNum(connectionInfo.getAuthToken()),
										ctx);
							}
						}
					} else {
...
//						}
						...
						if (!mConnectionInfoMap.containsKey(ctx.channel().id())) {
							...
							// Connector Channel Map 등록
							mConnectionChannelInfoMap.put(connectionInfo.getAuctionHouseCode() + "_" + JwtCertTokenUtils.getInstance().getUserMemNum(connectionInfo.getAuthToken()), ctx);
						}
					...
					if (mStandChannelsMap.containsKey(connectionInfo.getAuctionHouseCode()) && mStandChannelsMap.get(connectionInfo.getAuctionHouseCode()).contains(ctx.channel())) {
						...
						if (mAuctionScheduler.getCurrentAuctionStatus(connectionInfo.getAuctionHouseCode())
							...
							if (mAuctionScheduler.getAuctionState(connectionInfo.getAuctionHouseCode()) != null) {
								// 경매 유형코드 전송
								if (mAuctionScheduler.getAuctionEditSetting(connectionInfo.getAuctionHouseCode()) != null) {
									ctx.channel().writeAndFlush(new AuctionType(mAuctionScheduler.getAuctionEditSetting(connectionInfo.getAuctionHouseCode()).getAuctionHouseCode(), mAuctionScheduler.getAuctionEditSetting(connectionInfo.getAuctionHouseCode()).getAuctionType()).getEncodedMessage() + "\r\n");
								}
