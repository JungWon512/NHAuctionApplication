...
@Sharable
public final class AuctionServerDecodedRefreshConnectorHandler extends SimpleChannelInboundHandler<RefreshConnector> {
	private final Logger mLogger = LoggerFactory.getLogger(AuctionServerDecodedRefreshConnectorHandler.class);
	...
	public AuctionServerDecodedRefreshConnectorHandler(AuctionServer auctionServer, Auctioneer auctionSchedule,
	...
	@Override
	protected void channelRead0(ChannelHandlerContext ctx, RefreshConnector refreshConnector) throws Exception {
		if (mControllerChannelsMap.get(refreshConnector.getAuctionHouseCode()).contains(ctx.channel()) == true) {
			mLogger.info("정상 채널에서 응찰 접속자 정보를 요청하였습니다.");
			mAuctionServer.itemAdded(refreshConnector.getEncodedMessage());
		} else {
			mLogger.info("비정상 채널에서 응찰 접속자 정보를 요청하였으나, 요청이 거부되었습니다.");
		}
	}
}
