...
@Sharable
public final class AuctionServerDecodedRetryTargetInfoHandler extends SimpleChannelInboundHandler<RetryTargetInfo> {
	private final Logger mLogger = LoggerFactory.getLogger(AuctionServerDecodedRetryTargetInfoHandler.class);
	...
	public AuctionServerDecodedRetryTargetInfoHandler(AuctionServer auctionServer, Auctioneer auctionSchedule,
	...
	@Override
	protected void channelRead0(ChannelHandlerContext ctx, RetryTargetInfo retryTargetInfo) throws Exception {
		mLogger.info((retryTargetInfo.getEntryNum() + "출품번호 재경매"));
		mLogger.info(("재경매 대상 참여자번호 : " + retryTargetInfo.getRetryTargetInfo()));

		if (mControllerChannelsMap.get(retryTargetInfo.getAuctionHouseCode()).contains(ctx.channel()) == true) {
			mLogger.info("정상 채널에서 재경매 대상 정보를 전송하였습니다.");
			mAuctionServer.itemAdded(retryTargetInfo.getEncodedMessage());
		} else {
			mLogger.info("비정상 채널에서 재경매 대상 정보를 전송하였으나, 해당 처리 요청이 거부되었습니다.");
		}
	}
}
