...
@Sharable
public final class AuctionServerDecodedToastMessageRequestHandler
		extends SimpleChannelInboundHandler<ToastMessageRequest> {
	private final Logger mLogger = LoggerFactory.getLogger(AuctionServerDecodedToastMessageRequestHandler.class);
	...
	public AuctionServerDecodedToastMessageRequestHandler(AuctionServer auctionServer, Auctioneer auctionSchedule,
	...
	@Override
	protected void channelRead0(ChannelHandlerContext ctx, ToastMessageRequest toastMessageRequest) throws Exception {
		if (mControllerChannelsMap.get(toastMessageRequest.getAuctionHouseCode()).contains(ctx.channel()) == true) {
			mLogger.info("정상 채널에서 메시지 전송 요청이 되었습니다.");
			mAuctionServer.itemAdded(toastMessageRequest.getEncodedMessage());
		} else {
			mLogger.info("비정상 채널에서 메시지 전송을 요청하였으나, 요청이 거부되었습니다.");
		}
	}
}
