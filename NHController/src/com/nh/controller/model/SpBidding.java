...
public class SpBidding implements FromAuctionCommon, Serializable, Comparable<SpBidding> {
	...
	private StringProperty userNo;
	private StringProperty price;
	private StringProperty entryNum;
	...
	public String getBiddingInfoForLog() {
		return String.format("%c%c%c%s%c%s%c%s%c%s%c%s%c%s%c%s", ORIGIN, TYPE, AuctionShareSetting.DELIMITER, getAuctionHouseCode().getValue(), AuctionShareSetting.DELIMITER, getChannel().getValue(), AuctionShareSetting.DELIMITER, getUserNo().getValue(), AuctionShareSetting.DELIMITER,
				getEntryNum().getValue(), AuctionShareSetting.DELIMITER, getPrice().getValue(), AuctionShareSetting.DELIMITER, getIsNewBid().getValue(), AuctionShareSetting.DELIMITER, getBiddingTime().getValue());
	}
