...
import com.nh.share.api.response.ResponseApplicationVersion;
...
/**
 * 응찰 프로그램 버전 확인 요청 API명 : getApplicationVersion 응찰 프로그램 최신 버전 정보를 조회한다.
 *
 */
public class ActionRequestApplicationVersion extends Action {
     ...
     */
    public ActionRequestApplicationVersion(String appType, ActionResultListener resultListener) {
        mAppType = appType;
        mResultListenerBase = resultListener;
    }

    public interface RetrofitAPIService {
        @GET(GlobalDefine.API_GET_APPLICATION_VERSION)
        Call<ResponseApplicationVersion> getApplicationVersion(@Query("appType") String appType);
    }

    private final Callback<ResponseApplicationVersion> mCallBack = new Callback<ResponseApplicationVersion>() {
        @Override
        public void onResponse(Call<ResponseApplicationVersion> call, Response<ResponseApplicationVersion> response) {
            actionDone(resultType.ACTION_RESULT_RUNNEXT);
            Headers headers = response.headers();
            String type = headers.get(CONTENT_TYPE);
            ResponseApplicationVersion body = response.body();
        ...
        @Override
        public void onFailure(Call<ResponseApplicationVersion> call, Throwable t) {
    ...
    public void run() {
        ...
        mRetrofitAPIService.getApplicationVersion(mAppType).enqueue(mCallBack);
    }
}
