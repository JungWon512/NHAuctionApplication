...
import com.nh.share.api.response.ResponseAuctionScheduleInfo;
...
/**
 * 경매 일정 정보 조회 요청(캘린더) API명 : getAuctionScheduleInfo 경매 일정 정보를 조회한다.(캘린더 정보 조회)
 *
 */
public class ActionRequestAuctionScheduleInfo extends Action {
    private String mBaseDate; // 기준 연월

    /**
     * @param baseDate       경매 연월
     * @param resultListener
     */
    public ActionRequestAuctionScheduleInfo(String baseDate, ActionResultListener resultListener) {
        mBaseDate = baseDate;
        mResultListenerBase = resultListener;
    }

    public interface RetrofitAPIService {
        @GET(GlobalDefine.API_GET_AUCTION_SCHEDULE_INFO)
        Call<ResponseAuctionScheduleInfo> getAuctionScheduleInfo(@Query("baseDate") String baseDate);
    }

    private final Callback<ResponseAuctionScheduleInfo> mCallBack = new Callback<ResponseAuctionScheduleInfo>() {
        @Override
        public void onResponse(Call<ResponseAuctionScheduleInfo> call, Response<ResponseAuctionScheduleInfo> response) {
            actionDone(resultType.ACTION_RESULT_RUNNEXT);
            Headers headers = response.headers();
            String type = headers.get(CONTENT_TYPE);
            ResponseAuctionScheduleInfo body = response.body();
        ...
        @Override
        public void onFailure(Call<ResponseAuctionScheduleInfo> call, Throwable t) {
    ...
    public void run() {
        ...
        mRetrofitAPIService.getAuctionScheduleInfo(mBaseDate).enqueue(mCallBack);
    }
}
