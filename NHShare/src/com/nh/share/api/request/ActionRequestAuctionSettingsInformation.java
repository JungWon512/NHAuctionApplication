...
/**
 * 경매 생성 정보 조회 요청 API명 : getAuctionSettingsInformations 경매 생성 정보를 조회한다.
 *
 */
public class ActionRequestAuctionSettingsInformation extends Action {
    private String mAuctionCode; // 경매 구분 코드
    private String mAuctionRound; // 경매 회차 정보
    private String mAuctionLaneCode; // 경매 Lane 코드

    /**
     * @param auctionDateTime 기준 일시
     * @param auctionType     경매 종류
     * @param resultListener
     */
    public ActionRequestAuctionSettingsInformation(String auctionCode, String auctionRound, String auctionLaneCode,
    ...
    public interface RetrofitAPIService {
        @GET(GlobalDefine.API_GET_AUCTION_SETTINGS_INFORMATIONS)
        Call<ResponseAuctionSettingsInformation> getAuctionSettingsInformations(
                @Query("auctionCode") String auctionCode, @Query("auctionRound") String auctionRound,
                @Query("auctionLaneCode") String auctionLaneCode);
    }

    private final Callback<ResponseAuctionSettingsInformation> mCallBack = new Callback<ResponseAuctionSettingsInformation>() {
        @Override
        public void onResponse(Call<ResponseAuctionSettingsInformation> call,
                Response<ResponseAuctionSettingsInformation> response) {
            actionDone(resultType.ACTION_RESULT_RUNNEXT);
            Headers headers = response.headers();
            String type = headers.get(CONTENT_TYPE);
            ResponseAuctionSettingsInformation body = response.body();
        ...
        @Override
        public void onFailure(Call<ResponseAuctionSettingsInformation> call, Throwable t) {
    ...
    public void run() {
        ...
        mRetrofitAPIService.getAuctionSettingsInformations(mAuctionCode, mAuctionRound, mAuctionLaneCode)
                .enqueue(mCallBack);
    }
}
